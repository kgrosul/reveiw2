...
def reply_to_doc(message):
    bot.send_message(message.chat.id, 'Текст какого документа надо вывести?')
    user_request[message.chat.id] = 'doc'


@bot.message_handler(commands=['describe_doc'])
def reply_to_describe_doc(message):
    bot.send_message(message.chat.id, 'Статистику по какому документу надо вывести?')
    user_request[message.chat.id] = 'describe_doc'


@bot.message_handler(commands=['describe_topic'])
def reply_to_describe_topic(message):
    bot.send_message(message.chat.id, 'Статистику по какой теме надо вывести?')
    user_request[message.chat.id] = 'describe_topic'


@bot.message_handler(commands=['tag'])
def reply_to_describe_topic(message):
    bot.send_message(message.chat.id, 'Документы с какими тегами надо вывести?')
    user_request[message.chat.id] = 'tag'


@bot.message_handler(commands=['fun'])
def reply_to_describe_topic(message):
    bot.send_message(message.chat.id, 'Какая тема?)')
    user_request[message.chat.id] = 'fun'
...
def reply_to_text(message):
    if user_request[message.chat.id] == 'new_docs':
        ...
        if description is not None:
            ...
            bot.send_message(message.chat.id, "Введите корректное название темы")
        ...
        if text is not None:
            ...
            bot.send_message(message.chat.id, "Введите корректное название документа")
        ...
        if result:
            bot.send_message(message.chat.id, "Средняя длина документа: " +
                             str(make_request.get_avg_document_len(message.text)))
            bot.send_message(message.chat.id, "Всего документов: " +
                             str(make_request.get_documents_number(message.text)))
            with open(plot1_file_name, 'rb') as plot1:
                bot.send_photo(message.chat.id, plot1)
            with open(plot2_file_name, 'rb') as plot2:
                bot.send_photo(message.chat.id, plot2)
...
if __name__ == '__main__':
    while True:
        try:
            ...
            print("А вот сейчас бот упал из-за pollong")
